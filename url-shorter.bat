@echo off && title URL-Shorter_v0.2.2x [ConzZah] rem done @ 3:05 am / 18.02.2024
...
cls && echo QR-CODE: && echo -------- && curl https://qrenco.de/"%user_url%" && echo.
echo SHORTLINK: && echo ---------- && type "%c2c%" && clip < "%c2c%" && del /f /q "%c2c%"
:saveprompt
echo. && echo. && echo SAVE QR-CODE AS .PNG ? && echo ----------------------
...
echo [ QR-CODE SAVED TO: "%savepath%%savefolder%" ] && echo. && goto end